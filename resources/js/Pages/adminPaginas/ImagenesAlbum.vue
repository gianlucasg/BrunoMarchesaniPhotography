	...
	<v-app class="fondo">
		<div>
			<v-container class="hidden-sm-and-down">
				<v-row>
					<v-col align="start" justify="center" class="my-3" cols="12" xs="12" sm="12" md="4" lg="4">
						<v-btn  @click="atras"  x-large rounded dark color='#e4b747'>
							<v-icon left color="white">mdi-reply</v-icon> <span style="color:white">atrás</span>
						</v-btn>
					</v-col>
					<v-col align="center" justify="center" cols="12" xs="12" sm="12" md="4" lg="4">
						<v-pagination
					...
					<v-col align="end" justify="center" cols="12" xs="12" sm="12" md="4" lg="4">
			...
			<v-container class="hidden-md-and-up" >
				<v-row>
					...
					<v-col align="center" justify="center" class="my-3" cols="12" xs="12" sm="12" md="4" lg="4">
						<v-btn  @click="atras"  x-large rounded dark color="#511d47">
							<v-icon left color="white">mdi-reply</v-icon> <span style="color:white">atrás</span>
						</v-btn>
					</v-col>
				</v-row>
		    ...
		    <v-container>
		      <v-layout row wrap>
		        <v-col cols="12" xs="12" sm="6" md="6" lg="3" v-for="imagen in imagenes.data" :key="imagen.id">
...
export default {
  ...
  methods:{
		...
		modificar(id,titulo,descripcion,picker,imagen){
	  	...
	  	atras(){
			this.$inertia.visit(this.ruta.slice(0,this.ruta.lastIndexOf('/')));
		},
		subir(data){
			this.$root.$emit("iniciareventocargar");
  ...
  mounted(){
	...
	this.ruta= window.location.pathname.slice(0, window.location.pathname.indexOf(this.$props.id) + this.$props.id.length );
	this.imagenes = this.$props.imagns;
	this.verificarAlerta();
	this.$root.$emit('child-event',"album");
  }
